...
# It also calculates distances between region centroids
# It then generates a PyMOL visualization script .pml for showing the normal vectors, distances, and colored regions
...
def calculate_distances(centroids):
    ...
    valid_centroids = {name: centroid for name, centroid in centroids.items() if centroid is not None}
    
    # Calculate distances between each pair
    for (name1, c1), (name2, c2) in itertools.combinations(valid_centroids.items(), 2):
        distance = np.linalg.norm(c1 - c2)  # Euclidean distance
        distances[(name1, name2)] = distance
        
    return distances

def generate_pymol_script(pdb_file, regions, normals, centroids, angles, distances, output_file="visualization.pml"):
    """Generate a PyMOL script for visualizing the structure, normal vectors, and distances."""
...
cmd.group('normal_vectors', 'normal_vector_{region_name}')
        ...
        # Add centroid markers for each region
        f.write("\n# Region centroids\n")
        f.write("group centroids\n")
        for region_name, centroid in centroids.items():
            if centroid is not None:
                color = colors[region_name]
                f.write(f"""
# Centroid for {region_name}
pseudoatom centroid_{region_name}, pos=[{centroid[0]}, {centroid[1]}, {centroid[2]}]
set sphere_scale, 0.8, centroid_{region_name}
show spheres, centroid_{region_name}
color {color}, centroid_{region_name}
group centroids, centroid_{region_name}
                """)
        
        # Add distances between centroids
        if distances:
            f.write("\n# Distance measurements\n")
            f.write("group distance_measurements\n")
            ...
            for (region1, region2), distance in distances.items():
                ...
                f.write(f"""
# Distance between {region1} and {region2}
distance dist_{region1}_{region2}, centroid_{region1}, centroid_{region2}
color gray40, dist_{region1}_{region2}
set label_size, 24, dist_{region1}_{region2}
label dist_{region1}_{region2}, "{distance:.2f}\\u212b"
set dash_width, 2.0, dist_{region1}_{region2}
set dash_gap, 0.2, dist_{region1}_{region2}
group distance_measurements, dist_{region1}_{region2}
                """)
        ...
        f.write("set seq_view_format, 0\n")  # One-letter code
        f.write("set seq_view_label_spacing, 4\n")  # Spacing
        ...
        if angles:
            ...
            for (region1, region2), angle in angles.items():
                ...
                f.write(f"label {marker_name}, '{region1}-{region2}: {angle:.2f}\\u00b0'\n")
                f.write(f"set label_position, [0, 0, {10 + y_offset}], {marker_name}\n")
                f.write(f"set label_size, 24, {marker_name}\n")
        ...
        f.write("""
# Print instructions
print("\\nNormal vectors and distance visualization loaded successfully")
...
print("  - centroids: Group containing all region centroids")
print("  - distance_measurements: Group containing all distance measurements")
...
""")
        ...
        if distances:
            f.write("print(\"\\nCalculated distances between region centroids:\")\n")
            for (region1, region2), dist in distances.items():
                f.write(f"print(\"  - {region1} to {region2}: {dist:.2f}\\u212b)\n")
            f.write("print(\"  (These are also displayed as dashed lines with labels)\")\n")

        f.write("""
print("\\nTo adjust visibility, try commands like:")
print("  - set dash_radius, 0.8, normal_*")
print("  - set dash_color, marine, normal_*")
print("  - hide centroids # To hide all centroid spheres")
print("  - show centroids # To show all centroid spheres")
print("  - hide distance_measurements # To hide all distance measurements")
print("  - show distance_measurements # To show all distance measurements")
...
def main():
    ...
    if distances:
        ...
        for (region1, region2), distance in distances.items():
            print(f"  {region1} to {region2}: {distance:.2f}\u212b")
    else:
        print("\nCannot calculate distances: less than two valid centroids found")
    
    # Generate PyMOL script
    pymol_file = os.path.splitext(pdb_file)[0] + ".pml"
    generate_pymol_script(pdb_file, regions, normals, centroids, angles, distances, pymol_file)
    print(f"\nPyMOL visualization script generated: {pymol_file}")
    print("Load this script in PyMOL to visualize the structure, normal vectors, and distances.")
